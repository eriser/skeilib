#ifndef skei_widget_circular_waveform_included
#define skei_widget_circular_waveform_included
//----------------------------------------------------------------------
...
class SWidget_CircularWaveform
: public SWidget {
  ...
  public:
    ...
    SWidget_CircularWaveform(SRect ARect, uint32 AAlignment=swa_none)
    : SWidget(ARect,AAlignment) {
      MName         = "SWidget_CircularWaveform";
      MHint         = "circular aveform";
      MBuffer       = SKEI_NULL;//(float*)SMalloc(NUMSTEPS*sizeof(float));
      MBufferSize   = 0;//1024;
      //MRadius       = 100.0f;
      MOuterRadius  = 1.0f;
      MInnerRadius  = 0.6f;
    ...
    virtual ~SWidget_CircularWaveform() {
  ...
  public:
    ...
    void on_paint(SPainter* APainter, SRect ARect, uint32 AMode=0) {
      ...
      float radius = SMinI(MRect.w,MRect.h);
      ...
      //float prevx = cosf(angle*SKEI_PI2) * r_center;
      //float prevy = sinf(angle*SKEI_PI2) * r_center;
      ...
      if (MBipolar) {
        for (int32 i=0; i<NUMSTEPS; i++) {
          ...
          xx = cosf(angle*SKEI_PI2);
          yy = sinf(angle*SKEI_PI2);
