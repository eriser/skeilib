#ifndef skei_fft_included
#define skei_fft_included
//----------------------------------------------------------------------
...
class SFFT {
    ...
    */
    ...
    void create(int32 ASize) {
      ...
      if (ASize>0) {
        ...
        SMemset(MFftBuffer,0,num_bytes);
      }
    }
  ...
  public:
    ...
    void process(float* ABuffer) {
      SAssert(MFft!=SKEI_NULL);
      if (MFft) {
        pffft_transform_ordered(MFft,ABuffer,MFftBuffer,SKEI_NULL,PFFFT_FORWARD);
      }
    }
...
void* SMallocAligned(size_t nb_bytes) {
  return pffft_aligned_malloc(nb_bytes);
}

//----------

void  SMallocFree(void* ptr) {
