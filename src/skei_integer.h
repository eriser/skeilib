...
int SAverageI(const unsigned int n, const int* ar) {
  int total = 0;
  unsigned int i=0;
  while (i<n) {
    total += ar[i];   i++;
...
uint32 SBitReverse(uint32 v) {
  uint32 r = v;
  int s = sizeof(v) * SKEI_CHAR_BIT - 1;
  for (v >>= 1; v; v >>= 1) {
    r <<= 1;
    r |= v & 1;
    s--;
...
int SClampI(const int input, const int limit) {
...
int SMaxI(const int a, const int b) {
  return (a > b) ? a : b;
}
...
int SMinI(const int a, const int b) {
  return (a < b) ? a : b;
}
...
uint32 SNextPowerOfTwo(uint32 x) {
  x--;
...
int SRmsI(const unsigned int n, const int* ar) {
  int numr = 0;
  unsigned int i=0;
  while (i<n) {
    numr += ar[i]*ar[i];
