#ifndef skei_waveform_druttis
#define skei_waveform_druttis
//----------------------------------------------------------------------
...
class SWaveform_Sin_Druttis {
  ...
  public:
    ...
    float process(float t, float dt) {
      float f = 1.0f;//dt;
      return sin( (((t*f)+_ps)*SKEI_PI2) + (SKEI_PI05*_fm*afact(f)) );
    }
...
void SWaveform_Druttis_setSampleRate(float sr) {
  _sr = sr;
}
...
float SWaveform_Sine_Druttis(float t, float f, float ps, float fm) {
  return sin( (((t*f)+ps)*SKEI_PI2) + (SKEI_PI05*fm*_afact(f)) );
}

//----------

float SWaveform_Saw_Druttis(float t, float f, float ps, float fm, float fb) {
  float result = SWaveform_Sine_Druttis(t,f,ps,fb*SWaveform_Sine_Druttis(_prev_t/*t-1*/,f,ps,fm));
  _prev_t = t;
  return result;
}
...
float SWaveform_Pulse_Druttis(float t, float f, float ps, float fm, float fb, float pm, float pw) {
  return SWaveform_Saw_Druttis(t,f,ps,fm,fb) - SWaveform_Saw_Druttis(t,f,ps+(0.5*pw),fm,fb) * pm;
}
