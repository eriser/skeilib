#ifndef skei_bresenham_included
#define skei_bresenham_included
//----------------------------------------------------------------------
...
void setPixel(int x, int y) {
  uint32 c = SRgba(SWhite);
  setPixel(x,y,c);
}

void setPixelAA(int x, int y, int v) {
  uint32 c = SRgba(SWhite);
  v = 255 - v;
  blendPixel(x,y,c,v);
}

//void setPixelColor(int x, int y, int v) {
//  uint32 c = SRgba(SBrightYellow);
//  v = 255 - v;
//  blendPixel(x,y,c,v);
//}
...
void drawOptimizedEllipse(int xm, int ym, int a, int b);
void drawEllipseRect(int x0, int y0, int x1, int y1);
void drawEllipseRectAA(int x0, int y0, int x1, int y1);
...
void drawCircleAA(int xm, int ym, int r);
...
void drawQuadRationalBezierSegAA(int x0, int y0, int x1, int y1, int x2, int y2, float w);
...
void drawCubicBezierSegAA(int x0, int y0, float x1, float y1, float x2, float y2, int x3, int y3);

void drawQuadSpline(int n, int x[], int y[]);
void drawCubicSpline(int n, int x[], int y[]);
...
{                                  /* draw a limited quadratic Bezier segment */
  ...
  drawLine(x0,y0, x2,y2);                       /* draw remaining part to end */
}
...
{                   /* draw a limited rational Bezier segment, squared weight */
  ...
  if (cur != 0.0 && w > 0.0) {                            /* no straight line */
    ...
    if (w < 0.5 && (dy > xy || dx < xy)) {   /* flat ellipse, algorithm fails */
       ...
       drawQuadRationalBezierSeg(x0,y0, dx,dy, sx,sy, cur);/* draw separately */
       dx = floor((w*x1+x2)*xy+0.5); dy = floor((y1*w+y2)*xy+0.5);
       drawQuadRationalBezierSeg(sx,sy, dx,dy, x2,y2, cur);
       return;
    }
  ...
  drawLine(x0,y0, x2,y2);                     /* draw remaining needle to end */
}
...
{                                 /* draw any quadratic rational Bezier curve */
   ...
   if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
      ...
      drawQuadRationalBezierSeg(x0,y0, x,floor(yy+0.5), x,y, ww);
   ...
   if ((y0-y1)*(long)(y2-y1) > 0) {                    /* vertical cut at P6? */
      ...
      drawQuadRationalBezierSeg(x0,y0, floor(xx+0.5),y, x,y, ww);
      xx = (x1-x2)*(yy-y2)/(y1-y2)+x2;                /* intersect P7 | P1 P2 */
      x1 = floor(xx+0.5); x0 = x; y0 = y1 = y;            /* P0 = P6, P1 = P7 */
   }
   drawQuadRationalBezierSeg(x0,y0, x1,y1, x2,y2, w*w);          /* remaining */
}
...
{                                   /* draw ellipse rotated by angle (radian) */
   ...
   drawRotatedEllipseRect(x-a,y-b, x+a,y+b, (long)(4*zd*cos(angle)));
}
...
{                  /* rectangle enclosing the ellipse, integer rotation angle */
   ...
   float w = xd*(long)yd;
   if (zd == 0) return drawEllipseRect(x0,y0, x1,y1);          /* looks nicer */
   ...
   drawQuadRationalBezierSeg(x0,y0+yd, x0,y0, x0+xd,y0, 1.0-w);
   drawQuadRationalBezierSeg(x0,y0+yd, x0,y1, x1-xd,y1, w);
   drawQuadRationalBezierSeg(x1,y1-yd, x1,y1, x1-xd,y1, 1.0-w);
   drawQuadRationalBezierSeg(x1,y1-yd, x1,y0, x0+xd,y0, w);
}
...
{        /* draw a black anti-aliased rectangular ellipse on white background */
   ...
   if (a == 0 || b == 0) return drawLine(x0,y0, x1,y1);
   if (x0 > x1) { x0 = x1; x1 += a; }        /* if called with swapped points */
   if (y0 > y1) y0 = y1;                                  /* .. exchange them */
   y0 += (b+1)/2; y1 = y0-b1;                               /* starting pixel */
   a = 8*a*a; b1 = 8*b*b;
...
{                    /* draw an limited anti-aliased quadratic Bezier segment */
   ...
   {                                                      /* no straight line */
      ...
      do {
         ...
         setPixelAA(x0,y0, 255*fabs(err-dx-dy-xy)/ed);          /* draw curve */
   ...
   drawLineAA(x0,y0, x2,y2);                  /* draw remaining needle to end */
}
...
{   /* draw an anti-aliased rational quadratic Bezier segment, squared weight */
   ...
   if (cur != 0.0 && w > 0.0) {                           /* no straight line */
      ...
      if (w < 0.5 && dy > dx) {              /* flat ellipse, algorithm fails */
         ...
         sy = floor((y0+2.0*w*y1+y2)*xy/2.0+0.5);
         dx = floor((w*x1+x0)*xy+0.5); dy = floor((y1*w+y0)*xy+0.5);
         drawQuadRationalBezierSegAA(x0,y0, dx,dy, sx,sy, cur); /* draw apart */
         dx = floor((w*x1+x2)*xy+0.5); dy = floor((y1*w+y2)*xy+0.5);
         return drawQuadRationalBezierSegAA(sx,sy, dx,dy, x2,y2, cur);
      }
      ...
      do {                                                      /* pixel loop */
         ...
         if (x1 < 256) setPixelAA(x0,y0, x1);                   /* draw curve */
...
{                           /* draw limited anti-aliased cubic Bezier segment */
   ...
   if (xa == 0 && ya == 0) {                              /* quadratic Bezier */
      sx = floor((3*x1-x0+1)/2); sy = floor((3*y1-y0+1)/2);   /* new midpoint */
      return drawQuadBezierSegAA(x0,y0, sx,sy, x3,y3);
   }
   x1 = (x1-x0)*(x1-x0)+(y1-y0)*(y1-y0)+1;                    /* line lengths */
   x2 = (x2-x3)*(x2-x3)+(y2-y3)*(y2-y3)+1;
   do {                                                /* loop over both ends */
      ...
      for (fx = fy = f; x0 != x3 && y0 != y3; ) {
         ...
         if (y1 < 256) setPixelAA(x0, y0, y1);                  /* draw curve */
         ...
         if (2*fy <= f) {                           /* x+ anti-aliasing pixel */
            if (py < ed) setPixelAA(x0+sx, y0, 255*py/ed);      /* draw curve */
            y0 += sy; fy += f;                                      /* y step */
         }
         if (2*fx <= f) {                           /* y+ anti-aliasing pixel */
            if (px < ed) setPixelAA(x0, y2+sy, 255*px/ed);      /* draw curve */
            x0 += sx; fx += f;                                      /* x step */
         }
      }
...
exit:
   ...
   drawLineAA(x0,y0, x3,y3);     /* remaining part in case of cusp or crunode */
}
...
{                                    /* draw an anti-aliased line of width wd */
   ...
   for (wd = (wd+1)/2; ; ) {                                    /* pixel loop */
      setPixelAA/*Color*/(x0, y0, max(0,255*(abs(err-dx+dy)/ed-wd+1)));
      e2 = err; x2 = x0;
      if (2*e2 >= -dx) {                                            /* x step */
         for (e2 += dy, y2 = y0; e2 < ed*wd && (y1 != y2 || dx > dy); e2 += dx)
            setPixelAA/*Color*/(x0, y2 += sy, max(0,255*(abs(e2)/ed-wd+1)));
         if (x0 == x1) break;
         e2 = err; err -= dy; x0 += sx;
      }
      if (2*e2 <= dy) {                                             /* y step */
         for (e2 = dx-e2; e2 < ed*wd && (x1 != x2 || dx < dy); e2 += dy)
            setPixelAA/*Color*/(x2 += sx, y0, max(0,255*(abs(e2)/ed-wd+1)));
         if (y0 == y1) break;
         err += dx; y0 += sy;
      }
   }
...
{                         /* draw quadratic spline, destroys input arrays x,y */
   ...
   for (i = n-2; i > 0; i--) {                           /* back substitution */
      ...
      drawQuadBezier((x0+x1)/2,(y0+y1)/2, x1,y1, x2,y2);
      x2 = (x0+x1)/2; x1 = x0;
      y2 = (y0+y1)/2; y1 = y0;
   }
   drawQuadBezier(x[0],y[0], x1,y1, x2,y2);
}
...
{                             /* draw cubic spline, destroys input arrays x,y */
   ...
   drawCubicBezier(x3,y3, (x2+x4)/2,(y2+y4)/2, x4,y4, x4,y4);
   ...
   drawCubicBezier(x0,y0, x0,y0, (x0+x1)/2,(y0+y1)/2, x4,y4);
}
