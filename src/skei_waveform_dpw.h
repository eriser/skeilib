#ifndef skei_waveform_dpw
#define skei_waveform_dpw
//----------------------------------------------------------------------
...
class SWaveform_Saw_DPW2 {
  ...
  public:
    ...
    float process(float t, float dt) {
      // Correct phase, so it would be in line with sin(2.*SKEI_PI * t)
      t += (0.5f + (0.5f*dt));
      if (t >= 1) t -= 1;
