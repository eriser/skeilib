#ifndef skei_dmc_included
#define skei_dmc_included
//----------------------------------------------------------------------
/*
...
#define skei_dmc_threshold 2 // 8
#define skei_dmc_bigthresh 2 // 8
...
struct skei_dmc_node {
  float count[2];
  skei_dmc_node* next[2];
};
...
class SDMC {
  ...
  private:
    ...
    skei_dmc_node   *P;
    skei_dmc_node   *N;
    skei_dmc_node   nodes[256][256];
    skei_dmc_node   *nodebuf;
    skei_dmc_node   *nodemaxp;
    skei_dmc_node   *nodesptr;
  ...
  public:
    ...
    SDMC() {
      ...
      wbufpos = 0;
      memsize   = 0x1000000;
      threshold = skei_dmc_threshold;//2;
      bigthresh = skei_dmc_bigthresh;//2;
      nodebuf = SKEI_NULL;
    }
    ...
    void p_init(int memsize) {
      ...
      nodebuf = (skei_dmc_node *)SMalloc(memsize);
      //if (nodebuf == (skei_dmc_node *)NULL)
      //{
      //  fprintf(stderr,"memory alloc failed; try smaller predictor memory\n");
      //  exit(1);
      //}
      nodemaxp = nodebuf + (memsize/sizeof(skei_dmc_node)) - 20;
      p_flush();
    }
